...
If you survived that last section, this one will be cake, since we're basically doing the same thing, but for the `findOne(...)` method. So, let's create the `findOne(...)` function in our `MongoDB` class.

In your `MongoDB.js` class, add the following code to the class definition.
...
At the top of your `index.js` file – after the `const express = ...` require statement, add the following code:

```javascript
const ObjectID = require('mongodb').ObjectID;
```

Now let's create our new route. Add a new GET route for the `/api/users/:id` path. In the body of the function argument, add the following code:
...
This does the same thing as our last route, except it uses the new `findOne(...)` function and for the `query` object it is searching for users where `_id` is equal to `ObjectID(request.params.id)`. Again, because of the way MongoDB stored its IDs, we need to provide and instance of an `ObjectID` in our `query`. The constructor for the `ObjectID` class takes the string version of the ObjectID, which is the value that we will have in our request params, so we're good there.

Save your file, make a request to `/api/users`, and copy the value of the `_id` property on your user. Then append `/YOUR_COPIED_ID` to the URL, so that you're requesting `/api/users/YOUR_COPIED_ID`. In my case, it's `localhost:3000/api/users/59da90bf8dced62026fb914a`, but yours will be different, since the IDs are randomly generated by MongoDB.

When you request this URL, it'll likely look _**sort of**_ the same, but the key difference is that the response contains a single document object as opposed to an array of objects.

You've just pulled a document from MongoDB by ID!!!
